...
conifer3 <- read.csv('conifer1.csv') |> data.frame(name='conifer3', fill="green", color='darkgreen')
...
blob1 <- read.csv('blob.csv') |> data.frame(name='blob1', fill="#4DE600", color='darkgreen')
blob2 <- read.csv('blob.csv') |> data.frame(name='blob2', fill="#1A801A", color='darkgreen')
...
shapes <- rbind(conifer1,conifer2,conifer3,cloud1,flattop,flattop2,blob1,blob2,trunk,triangle,dome,sticks,grassy,forby,ferny,palm)
