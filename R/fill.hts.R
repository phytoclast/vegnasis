...
fill.hts.df <- function(df){
  ...
  return(df[,c("plot","label","symbol","taxon","type","nativity","cover","ht.min","ht.max","stratum.min","stratum.max","crown.min","crown.max","dbh.min","dbh.max","BA","stand.max","taxon.max")])
}
