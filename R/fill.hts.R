...
fill.hts.df <- function(df){
  ...
  return(df[,c("plot","label","name","symbol","taxon","type","nativity","cover","ht.min","ht.max","stratum.min","stratum.max","crown.min","crown.max","diam.min","diam.max","diam","BA","stand.max","taxon.max")])
}
