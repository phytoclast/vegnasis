...
extractTaxon <- function(rawnames, report = 'taxon'){
  ...
  x <- x |> mutate(rawname = str_replace_all(rawname,'^×\\s|^x\\s|^X\\s','×'),
                   ...
                   xxx = ifelse(grepl('\\sf.\\s[a-z]',xxx)|grepl('^f.\\s[a-z]',xxx) & !grepl('_f._ex\\s',xxx), str_replace_all(xxx, "\\sf.\\s|^f.\\s|^forma\\s|^nothof.\\s", '_f._' ),xxx),                         xxx1 = trimws(str_split_fixed(xxx,'_',3)[,1]),
